...
export function isValidPricePerWeek(price: string): boolean {
    return parsePrice(price) > 0 && ((price.toLowerCase().indexOf('per week') > -1 || price.toLowerCase().indexOf('pw') > -1));
}

export const calculateAveragePrice = function (listings: Listing[]) {
    if (listings.length === 0) return (0).toFixed(2);

    const prices = listings.filter((listing: Listing) => (listing.objective === 'rent') && isValidPricePerWeek(listing.priceDetails?.displayPrice))
